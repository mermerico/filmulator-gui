...
ImportModel::ImportModel(QObject *parent) : SqlModel(parent)
{
    ...
    dirNameFilters << "Raw image files (*.CR2 *.cr2 *.NEF *.nef *.DNG *.dng *.RW2 *.rw2 *.IIQ *.iiq *.ARW *.arw *.PEF *.pef *.RAF *.raf *.ORF *.orf *.SRW *.srw)"
                   << "All Image files (*.CR2 *.cr2 *.NEF *.nef *.DNG *.dng *.RW2 *.rw2 *.IIQ *.iiq *.ARW *.arw *.PEF *.pef *.RAF *.raf *.ORF *.orf *.SRW *.srw *.TIFF *.tiff *.TIF *.tif *.JPEG *.jpeg *.JPG *.jpg)";// << "All files (*)";
