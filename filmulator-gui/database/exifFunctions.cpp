...
QDateTime exifUtcTime(const std::string fullFilename, const int cameraTZ) {
  ...
  if (libraw_error) {
    ...
    cout << "exifUtcTime libraw error text: " << libraw_strerror(libraw_error)
         << endl;
    return QDateTime();
  }
  ...
  const int localTZ =
      QTimeZone::systemTimeZone().offsetFromUtc(QDateTime::currentDateTime());
...
QString exifLocalDateString(const std::string fullFilename, const int cameraTZ,
                            const int importTZ, const QString dirConfig) {
  QDateTime captureLocalDateTime;
  captureLocalDateTime.setOffsetFromUtc(importTZ);
  captureLocalDateTime.setSecsSinceEpoch(
      exifUtcTime(fullFilename, cameraTZ).toSecsSinceEpoch());
...
int exifDefaultRotation(const std::string fullFilename) {
  const bool isCR3 = QString::fromStdString(fullFilename)
...
QString fractionalTv(const float shutterSpeed) {
  ...
  float multError =
      abs(float(multDenominator) / (shutterSpeed * float(multNumerator)) - 1);
  ...
  float divError =
      abs(float(divDenominator) / (shutterSpeed * float(divNumerator)) - 1);
...
int exifRating(const std::string fullFilename) {
  if (QString::fromStdString(fullFilename)
          .endsWith(".cr3", Qt::CaseInsensitive)) {
    return 0;
  } else {
...
float nikonFocalLength(const unsigned int inputFL) {
  ...
  if (inputFL <= 35) {
    return 13;
  } // a coverage guess; there's no electronic 13mm lens to reference against
  if (inputFL <= 36) {
    return 14;
  }
  if (inputFL <= 39) {
...
QString nikonAperture(const unsigned int inputAperture) {
  if (inputAperture <= 7) {
    return "0.95";
  } // a coverage guess; I don't know what the Z 50/.95 is
  if (inputAperture <= 8) {
    return "1.2";
  }
  if (inputAperture <= 12) {
  ...
  if (inputAperture <= 102) {
    return "19";
  }
  if (inputAperture <= 108) {
    return "22";
  }
  return "";
}
