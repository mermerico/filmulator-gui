...
#pragma omp parallel
    {
        ...
        for(int row = 0; row < nrows; row++) {
            for(int col = 0; col<ncols; col++) {
                ...
                const float rSurplus = rsurp - crosstalkToe + (crosstalkToe*crosstalkToe)/(rsurp + crosstalkToe + 1/65535.0f);
                const float gSurplus = gsurp - crosstalkToe + (crosstalkToe*crosstalkToe)/(gsurp + crosstalkToe + 1/65535.0f);
                const float bSurplus = bsurp - crosstalkToe + (crosstalkToe*crosstalkToe)/(bsurp + crosstalkToe + 1/65535.0f);
                const float maxSurplus = max(max(max(rSurplus, gSurplus), bSurplus), 0.000001f);
                const float sumSurplus = rSurplus+gSurplus+bSurplus;
                const float crosstalkR = inputR + sumSurplus*highlight_crosstalk - (rSurplus*rSurplus/maxSurplus)*highlight_crosstalk;
                const float crosstalkG = inputG + sumSurplus*highlight_crosstalk - (gSurplus*gSurplus/maxSurplus)*highlight_crosstalk;
                const float crosstalkB = inputB + sumSurplus*highlight_crosstalk - (bSurplus*bSurplus/maxSurplus)*highlight_crosstalk;
                //rolloff
                float rOutput = max(0.0f, crosstalkR - toe_boundary + (toe_boundary*toe_boundary)/(crosstalkR + toe_boundary+1/65535.0f));
                rOutput = rOutput > rolloff_boundary ? 65535.f - ((crystal_headroom * crystal_headroom) / (rOutput + crystal_headroom - rolloff_boundary)) : rOutput;
                input_image(row,colr) = rOutput * crystals_per_pixel;
                float gOutput = max(0.0f, crosstalkG - toe_boundary + (toe_boundary*toe_boundary)/(crosstalkG + toe_boundary+1/65535.0f));
                gOutput = gOutput > rolloff_boundary ? 65535.f - ((crystal_headroom * crystal_headroom) / (gOutput + crystal_headroom - rolloff_boundary)) : gOutput;
                input_image(row,colg) = gOutput * crystals_per_pixel;
                float bOutput = max(0.0f, crosstalkB - toe_boundary + (toe_boundary*toe_boundary)/(crosstalkB + toe_boundary+1/65535.0f));
                bOutput = bOutput > rolloff_boundary ? 65535.f - ((crystal_headroom * crystal_headroom) / (bOutput + crystal_headroom - rolloff_boundary)) : bOutput;
