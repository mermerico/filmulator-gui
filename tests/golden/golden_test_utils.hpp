...
template <typename T>
bool saveGolden(const matrix<T>& img, const std::string& path) {
...
template <typename T>
bool loadGolden(matrix<T>& img, const std::string& path) {
...
template <typename T>
CompareResult compare(const matrix<T>& output, const matrix<T>& golden,
    ...
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            float diff =
                std::abs(static_cast<float>(output(r, c)) - static_cast<float>(golden(r, c)));
            sumDiff += diff;
            if (diff > maxDiff) {
                maxDiff = diff;
            }
...
template <typename T>
CompareResult compareToGolden(const matrix<T>& output, const std::string& goldenPath,
                              float maxTolerance = CompareResult::DEFAULT_MAX_TOLERANCE,
                              float avgTolerance = CompareResult::DEFAULT_AVG_TOLERANCE) {
    matrix<T> golden;
    if (!loadGolden(golden, goldenPath)) {
        CompareResult result = {std::numeric_limits<float>::max(),
                                std::numeric_limits<float>::max(), false, -1};
        return result;
    }
...
template <typename T>
void createSolidImage(matrix<T>& img, int rows, int cols, T value) {
    img.set_size(rows, cols);
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            img(r, c) = value;
...
template <typename T>
void createSolidRGB(matrix<T>& img, int height, int width, T r, T g, T b) {
    int rows = height;
...
template <typename T>
void createHorizontalGradient(matrix<T>& img, int rows, int cols, T minVal, T maxVal) {
    img.set_size(rows, cols);
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
...
template <typename T>
void createVerticalGradient(matrix<T>& img, int rows, int cols, T minVal, T maxVal) {
    img.set_size(rows, cols);
    for (int r = 0; r < rows; r++) {
...
template <typename T>
void createRGBGradient(matrix<T>& img, int height, int width, T maxVal) {
    int rows = height;
    int cols = width * 3;
    img.set_size(rows, cols);
...
template <typename T>
void createCheckerboard(matrix<T>& img, int rows, int cols, int cellSize, T color1, T color2) {
    img.set_size(rows, cols);
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
...
template <typename T>
void createNeutralGray(matrix<T>& img, int height, int width, T grayLevel) {
